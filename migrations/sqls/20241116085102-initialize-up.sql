...
create table users
(
    email             text primary key,               -- using text as recommended
    admin             boolean not null default false,
    name              text    not null,
    password          text    not null,
    remember_token    text,
    editor            text    not null default 'default',
...
(
    ...
    constraint projects_user_email_fk foreign key (user_email) references users (email) on delete cascade -- if user is deleted, delete his projects
);
...
(
    ...
    category                  text not null default 'uncategorized', -- fk to categories (hascategory relation)
